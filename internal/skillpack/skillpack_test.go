...
func TestPackStandard_AuthoringContent(t *testing.T) {
	packed, err := PackStandard(testSkillsDir, "anvil-standard-flutter")
	if err != nil {
		t.Fatalf("pack the authored skills: %v", err)
	}
	if len(packed) < 2 {
		t.Fatalf("packed %d skills, want at least 2 (flutter-conventions, flutter-delivery)", len(packed))
	}
	...
	for _, want := range []string{"flutter-conventions", "flutter-delivery"} {
		if !names[want] {
...
func TestPackStandard_Deterministic(t *testing.T) {
	first, err := PackStandard(testSkillsDir, "anvil-standard-flutter")
	if err != nil {
		t.Fatalf("first pack: %v", err)
	}
	second, err := PackStandard(testSkillsDir, "anvil-standard-flutter")
	if err != nil {
...
func TestAssetID(t *testing.T) {
	id, err := AssetID("flutter-conventions", "1.0.0")
	if err != nil {
		t.Fatalf("AssetID: %v", err)
	}
	if id != "anvil-skill-flutter-conventions-1-0-0" {
		t.Errorf("AssetID = %q, want anvil-skill-flutter-conventions-1-0-0", id)
	}
	if _, err := AssetID("Bad_Name", "1.0.0"); err == nil {
		t.Error("AssetID accepted an invalid name")
	}
	if _, err := AssetID("flutter-conventions", "01.0.0"); err == nil {
