...
// Tests for the SOURCE manifest of the Flutter delivery lifecycle
// standard (standard/manifest.json): the authoring-time manifest fields
// (TS-018-02-03, 007 §4).
//
...
// package dir): internal/release -> ../../standard/manifest.json.
const sourceManifestPath = "../../standard/manifest.json"
...
func TestSourceManifest_FieldCompleteness(t *testing.T) {
	...
	if src.ID != "anvil-standard-flutter" {
		t.Errorf("id = %q, want the standard identity anvil-standard-flutter", src.ID)
	}
	if src.Version == "" {
...
// framework-version support scope (007 §4, ADR-021 §3.2): Flutter stable
// releases 3.0.0 through 3.32.0 — unique, well-formed, and exactly the
...
func TestSourceManifest_DeclaresFrameworkSupportScope(t *testing.T) {
	...
	want := []string{
		...
		"3.13.0", "3.16.0", "3.19.0", "3.22.0",
		"3.24.0", "3.27.0", "3.29.0", "3.32.0",
	}
...
func TestSourceManifest_DerivedDocumentPreservesDeclarations(t *testing.T) {
	...
	doc := DeriveDocument(src, src.Version,
		"https://github.com/maleolabs/anvil-standard-flutter/releases/download/v"+src.Version+"/anvil-standard-flutter-"+src.Version+".tar.gz",
