...
func TestAttestationPayload_Composition(t *testing.T) {
	...
	// id = "anvil-standard-flutter", version = "1.0.0", digest =
	// 64 x '0' (hex of 32 zero bytes).
	digest := strings.Repeat("0", 64)
	payload, err := AttestationPayload("anvil-standard-flutter", "1.0.0", []ContentDigest{{
	...
	want := []byte("anvil-standard-flutter")
	...
	if len(payload) != len("anvil-standard-flutter")+1+len("1.0.0")+1+32 {
...
func TestDeriveDocument_AddsReleaseTimeFields(t *testing.T) {
	// The Flutter source manifest (standard/manifest.json) intentionally
	// OMITS the release-time fields (distribution/lifecycle/trust —
	// standard/README.md parseability note). The derived document must
	// add them with real release values (TS-016-03-02); the release
	// pipeline never ships a document without them.
	src := &SourceManifest{
		Schema:          SchemaURN,
		Title:           "Flutter Delivery Lifecycle Standard",
		ID:              "anvil-standard-flutter",
		Version:         "1.0.0",
		ContractVersion: "1.0.0",
		Capability:      Capability{FrameworkVersion: []string{"3.0.0", "3.22.0", "3.32.0"}},
		// distribution, lifecycle, trust deliberately absent
	}
	doc := DeriveDocument(
		src,
		"1.0.0",
	...
	if doc.ID != "anvil-standard-flutter" || doc.Version != "1.0.0" || doc.ContractVersion != "1.0.0" {
...
func buildAttestedRelease(t *testing.T, content []byte, contentDigests []ContentDigest) (*MetadataDocument, []byte) {
	...
	payload, err := AttestationPayload("anvil-standard-flutter", "1.0.0", contentDigests)
	if err != nil {
		t.Fatalf("AttestationPayload: %v", err)
	}
	doc := DeriveDocument(
		&SourceManifest{ID: "anvil-standard-flutter", Version: "1.0.0", ContractVersion: "1.0.0", Capability: Capability{FrameworkVersion: []string{"10.0.0", "11.0.0", "12.0.0"}}},
		"1.0.0",
		"https://github.com/maleolabs/anvil-standard-flutter/releases/download/v1.0.0/anvil-standard-flutter-1.0.0.tar.gz",
...
func TestBinaryAssetDigests_ComputesSortedNamedEntries(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "anvil-adapter-flutter-darwin-arm64"), []byte("darwin arm64"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, "anvil-adapter-flutter-linux-amd64"), []byte("linux amd64"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if digests[0].Name != "anvil-adapter-flutter-darwin-arm64" || digests[1].Name != "anvil-adapter-flutter-linux-amd64" {
...
func TestVerifyDocumentAndBinaryDigests_Valid(t *testing.T) {
	...
	bins := map[string][]byte{
		"anvil-adapter-flutter-linux-amd64":  []byte("linux amd64 binary"),
		"anvil-adapter-flutter-linux-arm64":  []byte("linux arm64 binary"),
		"anvil-adapter-flutter-darwin-amd64": []byte("darwin amd64 binary"),
	}
	contentDigests := []ContentDigest{{
		Algorithm: DigestAlgorithmSHA256, Encoding: DigestEncodingBase16, Digest: SHA256Base16(content),
	}}
	names := []string{"anvil-adapter-flutter-darwin-amd64", "anvil-adapter-flutter-linux-amd64", "anvil-adapter-flutter-linux-arm64"}
...
func TestVerifyDocument_NamedEntriesNotComparedWithContent(t *testing.T) {
	...
	contentDigests := []ContentDigest{
		{Algorithm: DigestAlgorithmSHA256, Encoding: DigestEncodingBase16, Digest: SHA256Base16(content)},
		{Algorithm: DigestAlgorithmSHA256, Encoding: DigestEncodingBase16, Digest: SHA256Base16([]byte("binary bytes, not content")), Name: "anvil-adapter-flutter-linux-amd64"},
	}
	doc, content := buildAttestedRelease(t, content, contentDigests)
	if err := VerifyDocument(doc, content); err != nil {
...
func TestVerifyBinaryAssetDigests_TamperedBinary(t *testing.T) {
	dir := t.TempDir()
	name := "anvil-adapter-flutter-linux-amd64"
...
func TestVerifyBinaryAssetDigests_MissingAndUndeclaredAssets(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "anvil-adapter-flutter-linux-arm64"), []byte("undeclared"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.Remove(filepath.Join(dir, "anvil-adapter-flutter-linux-arm64")); err != nil {
		t.Fatal(err)
	}
	...
	doc2.Trust.ContentDigests = append(doc2.Trust.ContentDigests, ContentDigest{
		Algorithm: DigestAlgorithmSHA256, Encoding: DigestEncodingBase16,
		Digest: SHA256Base16([]byte("ghost")), Name: "anvil-adapter-flutter-darwin-arm64",
	})
...
func TestAttestationPayload_NamedEntryComposition(t *testing.T) {
	...
	payload, err := AttestationPayload("anvil-standard-flutter", "1.0.0", []ContentDigest{
		{Algorithm: DigestAlgorithmSHA256, Encoding: DigestEncodingBase16, Digest: d1},
		{Algorithm: DigestAlgorithmSHA256, Encoding: DigestEncodingBase16, Digest: d2, Name: "anvil-adapter-flutter-linux-amd64"},
	})
	if err != nil {
		t.Fatalf("AttestationPayload: %v", err)
	}
	...
	want = append(want, 0x00)
	want = append(want, bytes.Repeat([]byte{0x00}, 32)...)
	want = append(want, "anvil-adapter-flutter-linux-amd64"...)
	want = append(want, 0x00)
...
func TestVerifyAttestation_NameStripAndRenameInvalidate(t *testing.T) {
	...
	contentDigests := []ContentDigest{
		{Algorithm: DigestAlgorithmSHA256, Encoding: DigestEncodingBase16, Digest: SHA256Base16([]byte("archive"))},
		{Algorithm: DigestAlgorithmSHA256, Encoding: DigestEncodingBase16, Digest: SHA256Base16([]byte("binary")), Name: "anvil-adapter-flutter-linux-amd64"},
	}
	payload, err := AttestationPayload("anvil-standard-flutter", "1.0.0", contentDigests)
	if err != nil {
		t.Fatalf("AttestationPayload: %v", err)
	...
	renamed := *doc
	renamed.Trust.ContentDigests[1].Name = "anvil-adapter-laravel-linux-amd64"
...
func TestSignAndVerifyDocumentSignature(t *testing.T) {
	...
	docBytes := []byte(`{"id":"anvil-standard-flutter","version":"1.0.0","trust":{"contentDigests":[{"algorithm":"sha-256","encoding":"base16","digest":"1111111111111111111111111111111111111111111111111111111111111111","name":"anvil-adapter-flutter-linux-amd64"}],"attestation":{"algorithm":"ed25519","signature":"x","publicKey":"y"}}}`)
