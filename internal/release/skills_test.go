...
func skillsTestFragment(t *testing.T) *SkillsFragment {
	...
	return &SkillsFragment{
		Skills: []Skill{
			{Name: "flutter-conventions", Version: "1.0.0", Asset: "anvil-skill-flutter-conventions-1-0-0", Description: "conventions"},
			{Name: "flutter-delivery", Version: "1.0.0", Asset: "anvil-skill-flutter-delivery-1-0-0", Description: "delivery"},
		},
		Trust: SkillsTrustFragment{
			ContentDigests: []ContentDigest{
				{Algorithm: DigestAlgorithmSHA256, Encoding: DigestEncodingBase16, Digest: strings.Repeat("a", 64), Name: "anvil-skill-flutter-conventions-1-0-0"},
				{Algorithm: DigestAlgorithmSHA256, Encoding: DigestEncodingBase16, Digest: strings.Repeat("b", 64), Name: "anvil-skill-flutter-delivery-1-0-0"},
			},
		},
	}
}
...
func skillsTestDoc() *MetadataDocument {
	return &MetadataDocument{
		...
		Distribution:    Distribution{Type: DistributionTypeGitHubReleases, Location: "https://github.com/maleolabs/anvil-standard-flutter/releases/download/v1.1.1/anvil-standard-flutter-1.1.1.tar.gz"},
		...
		Trust: Trust{
			ContentDigests: []ContentDigest{
				...
				{Algorithm: DigestAlgorithmSHA256, Encoding: DigestEncodingBase16, Digest: strings.Repeat("d", 64), Name: "anvil-adapter-flutter-linux-amd64"},
			},
...
func TestMergeSkillsFragment_AppendsInOrder(t *testing.T) {
	...
	wantOrder := []string{"", "anvil-adapter-flutter-linux-amd64", "anvil-skill-flutter-conventions-1-0-0", "anvil-skill-flutter-delivery-1-0-0"}
...
func TestVerifySkillAssetDigests_Tampered(t *testing.T) {
	...
	name := "anvil-skill-flutter-conventions-1-0-0"
...
func TestVerifySkillAssetDigests_MissingAsset(t *testing.T) {
	...
	dir := skillsTestAssetsDir(t, doc)
	if err := os.Remove(filepath.Join(dir, "anvil-skill-flutter-delivery-1-0-0")); err != nil {
		t.Fatal(err)
	}
	if err := VerifySkillAssetDigests(doc, dir); err == nil {
