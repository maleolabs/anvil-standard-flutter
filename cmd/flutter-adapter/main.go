...
// template, manifest (005-adapter-command-contract §5.2, §6.2; the
// template command returns the adapter-owned pipeline definitions,
// ADR-020 §1). The `activate` command is intentionally absent: the hybrid
// deployment model has no server activation (TS-P7-20 AC-5, ADR-016,
// EPIC-007 §7.3). JSON result on stdout; exit 0 on a produced result,
// non-zero on dispatch failure (ADR-010 §8.1).
//
// Reference: TS-P7-20, TS-P7-21, TS-P7-22, TS-P7-25, TS-P7-26,
// TS-007-038, ADR-016, ADR-020, 004-review-resolutions D1
package main
