...
This guide explains how an operator registers `anvil-standard-flutter` with
...
```bash
mkdir -p ~/.config/anvil/registry/anvil-standard-flutter

curl -fL -o ~/.config/anvil/registry/anvil-standard-flutter/1.1.1.json \
  https://github.com/maleolabs/anvil-standard-flutter/releases/download/v1.1.1/registry-metadata-1.1.1.json
```
...
PUBKEY=$(jq -r '.trust.attestation.publicKey' \
  ~/.config/anvil/registry/anvil-standard-flutter/1.1.1.json)

jq -n --arg k "$PUBKEY" '{publishers: {"anvil-standard-flutter": $k}}' \
...
{
  "publishers": {
    "anvil-standard-flutter": "<base64 public key from .trust.attestation.publicKey>"
  }
}
```
...
anvil standard install anvil-standard-flutter   # version optional: resolves latest
...
- This standard's skills (`flutter-conventions`, `flutter-delivery`) are
