...
```sh
# 1. Bump the version line on develop: edit standard/manifest.json
...
4. **Package** — the release archive
   `anvil-standard-flutter-<version>.tar.gz`: the platform binaries plus
   the standard content (source manifest, manifest documentation, lifecycle
   definition, verification, templates, compatibility, docs, tests,
   license, and the authored skills). The archive is the release content
   resolved from `distribution.location` at adoption.
...
7. **Index (stable releases only)** — commits the registry metadata
   document to `registry/index/anvil-standard-flutter/<version>.json` on
...
| Artifact | Meaning |
|---|---|
| `anvil-standard-flutter-<version>.tar.gz` | The release content: platform binaries + standard parts (incl. the authored skills); the content `distribution.location` resolves and `trust.contentDigests` covers |
...
| `binaries/anvil-adapter-flutter-<os>-<arch>` | The standard executable per release platform |
...
the Core fixture
(`fixtures/standard-skills/anvil-standard-flutter/skills`); authoring
...
- **Integrity.** `trust.contentDigests` carries the real SHA-256 digest
  ...
  to its asset, e.g. `anvil-adapter-flutter-linux-amd64`) and per skill
...
- **Stable signing key (recommended for production).** Set the
  ...
  base64 -w0 ./keys/release-signing-key.pem | gh secret set RELEASE_SIGNING_KEY -R maleolabs/anvil-standard-flutter
  ```
...
# Inspect one release
anvil standard inspect anvil-standard-flutter 1.1.0 --index <checkout-of-this-repo>

# Install (explicit adoption: validation + integrity + attestation + record)
anvil standard install anvil-standard-flutter 1.1.0 --index <checkout-of-this-repo>
```
