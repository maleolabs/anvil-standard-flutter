...
| Artifact | Meaning |
|---|---|
| `anvil-standard-flutter-<version>.tar.gz` | The release content: platform binaries + standard parts; the content `distribution.location` resolves and `trust.contentDigests` covers |
...
- **Integrity.** `trust.contentDigests` carries the real SHA-256 digest
  ...
  to its asset, e.g. `anvil-adapter-flutter-linux-amd64`). At adoption
