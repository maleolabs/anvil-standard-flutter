#!/usr/bin/env bash
#
# validate-manifest.sh — validates the SOURCE manifest of the Flutter
# delivery lifecycle standard (standard/manifest.json) for INTERNAL
# consistency.
#
...
#      this standard's identity (anvil-standard-flutter);
...
#   The source manifest intentionally OMITS the release-time fields —
#   distribution, lifecycle and trust (standard/README.md parseability
#   note) — which the standard's release pipeline populates with real
#   values at publication (ADR-030, TS-016-03-02). Unlike the Laravel
#   standard's source manifest (which carries format-valid placeholder
#   trust fields), this source manifest omits them; until TS-016-03-02
#   fills the release-time fields, neither source manifest is a
#   publishable registry metadata document for the Core registry client's
#   strict parser (registry-metadata.schema.json, which requires
#   distribution, lifecycle and trust) — do not run it through the
#   registry parser. This script checks internal consistency only, so CI
#   stays green on the source manifest.
#
...
MANIFEST_DEFAULT="${ROOT_DIR}/standard/manifest.json"
EXPECTED_ID="anvil-standard-flutter"
    ...
    cat > "${SELFTEST_TMP}/valid.json" <<'EOF'
{
  ...
  "capability": { "frameworkVersion": ["3.0.0", "3.3.0", "3.7.0"] }
}
EOF
    ...
    jq '.capability.frameworkVersion = ["3.0.0", "3.0"]' "${SELFTEST_TMP}/valid.json" > "${SELFTEST_TMP}/bad-framework.json"
